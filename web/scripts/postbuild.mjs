...
// The font binaries are part of the static site; carry their upstream licenses.
await mkdir('dist/licenses', { recursive: true });
for (const family of ['geist', 'geist-mono']) {
  const directory = `node_modules/@fontsource-variable/${family}`;
  const license = (await readdir(directory)).find(name => /^(license|ofl)(\.(txt|md))?$/i.test(name));
  if (!license) throw new Error(`Missing upstream font license: ${family}`);
  await writeFile(`dist/licenses/${family}.txt`, await readFile(join(directory, license)));
}
