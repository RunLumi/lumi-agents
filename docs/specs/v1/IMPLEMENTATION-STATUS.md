...
| 30 | Document workspace preview/edit | **partial → advancing** | PR: markdown rendered/source/split (react-markdown+remark-gfm, HTML disabled, inert links), image preview via bounded base64 IPC, literal CSV/TSV grid, explicit unsupported-format states; text edit existed. Remaining per phase A–C: CodeMirror editor, PDF (pdfjs), DOCX/XLSX/PPTX adapters, staged-save orchestrator binding |
