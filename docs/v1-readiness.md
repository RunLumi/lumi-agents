...
- The `lumi-scheduler` crate already proves important scheduler safety primitives deterministically: cron-style schedule records, explicit timezone/catch-up policy, deduplication, quiet hours, device-availability checks, unattended leases, revocation, action budgets, kill-switch behavior, approval parking, and routing scheduled actions through the normal orchestrator gate. **This is scheduler-core evidence, not a finished Automations product.** Spec 13 now additionally requires durable Automation definitions/revisions, AT/INTERVAL/RRULE/EVENT + follow-up offsets, Project/task-source binding, run history, overlap/retry/auto-pause, semantic outcomes, manifest import, Review Queue, and desktop UX. Track the missing vertical slice in #73.
...
- Spec 26 now makes Folder-as-Project a V1 Work-mode requirement. PRs #54–#59 (Sep 2026) implement and gate the core slice: durable Project identity with on-disk marker proof (`lumi-project`, ADR 0011), Open Folder/Recent with deliberate relink, root-bounded file operations with stale-write protection and change sets, bounded Git (read/local-write only; push/destructive denied by policy), bounded shell validations with honest five-state outcomes, bounded discovery, a project-bound task store, and the desktop Work-mode UI. Project memory (provenance-required, head-staleness, invalidation) and Clone Repository (deterministic local-clone tests, cloned checkout opens as a project) landed with them. Deterministic tests cover spec 26 §26.34 items 1–22 and 25, plus an acceptance anchor (`tests/project_acceptance.rs`) running the §26.33 spine on one hostile-instruction fixture. NOT yet proven: a live end-to-end run in the shipped desktop binary on a dirty real internal repository (dogfood — see the BLOCKER comment on #50), and model-driven planning on top of the runtime.
...
| 22.8 Automations | `lumi-scheduler` has deterministic admission/dedup/catch-up/lease/revocation/approval safety tests | Implement durable Automation records/revisions, full trigger set, Project source hashing, overlap/retry/auto-pause, semantic outcomes, History/Review Queue, manifest import, restart, and Ads Agents acceptance (#73) |
...
| Project-native Automations | [#73](https://github.com/RunLumi/lumi-agents/issues/73) | Import/create durable Automation; survive restart; due/event occurrence creates real Task/Run; bounded unattended authority; dedup/overlap/catch-up/retry/auto-pause; semantic outcomes; History/Review Queue; Ads Agents scheduled fixture leaves report/worklog and zero unauthorized Ads writes |
