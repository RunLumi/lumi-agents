...
| Medium | Managed browser remains read-only; authenticated Chrome is a separate explicit selected-window path and still lacks mutation/upload/download support. | Partial / intentional boundary | Keep Chrome project-bound, origin-scoped and revocable; do not claim authenticated customer canary or browser mutation support. |
