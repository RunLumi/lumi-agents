...
| PPTX preview/basic edit | pptx-react-viewer / pptx-viewer-core, ChristopherVR | Apache-2.0 root; separately licensed bundled components | **Approved 2026-09-21** (explicit license review): MPL-2.0 `mtx-decompressor@1.6.0` accepted and recorded in the desktop MPL notices. Pinned `pptx-react-viewer@3.17.0` + required peers (the npm registry supply-chain date-cutoff excludes 4.1.1; upgrade when the cutoff passes). Static preview shipped read-only via the `./viewer` subpath; basic-edit qualification per the §30.10 supported profile remains a separate gate. |
