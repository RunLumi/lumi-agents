...
const UTILITY_STEMS =
  /^(p|px|py|pt|pb|pl|pr|ps|pe|m|mx|my|mt|mb|ml|mr|gap|w|h|min-w|max-w|min-h|max-h|size|top|bottom|left|right|inset|text|bg|border|border-b|border-t|border-l|border-r|rounded|font|leading|tracking|shadow|opacity|ring|ring-offset|transition|duration|ease|translate|translate-y|translate-x|z|col|row|order|flex|grid|items|justify|content|self|place|overflow|whitespace|outline|scrollbar|shrink|grow|basis|animate|filter|brightness|underline|selection|group|data|aria|from|to|via|list|object|aspect|columns|divide|space|table|cursor|select|resize|appearance|pointer-events|fill|stroke|sr)-|^(flex|grid|block|inline|inline-flex|hidden|relative|absolute|fixed|sticky|static|truncate|tabular-nums|sr-only|isolate|container|subgrid|antialiased|italic|uppercase|lowercase|capitalize|underline|line-through|no-underline|border|border-collapse|outline-none|appearance-none|overflow-ellipsis)$/;
